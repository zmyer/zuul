 ...
 */
@Singleton
public class NewFilterProcessorImpl
{
    protected static final Logger LOG = LoggerFactory.getLogger(NewFilterProcessorImpl.class);
    ...
    @Inject
    public NewFilterProcessorImpl(FilterLoader loader, FilterUsageNotifier usageNotifier)
    {
        filterLoader = loader;
        this.usageNotifier = usageNotifier;
    }
    
    public ZuulMessage applyFilterChain(ZuulMessage msg, FilterComplete callback)
    {
        msg = applyInboundFilters(msg, chain);
        msg = applyEndpointFilter(msg, chain);
        msg = applyOutboundFilters(msg, chain);
        
        return msg;
    }

    protected ZuulMessage applyInboundFilters(final ZuulMessage message, final Observable<ZuulMessage> chain) {
        return applyFilters(message, FilterType.INBOUND);
    }

    protected ZuulMessage applyOutboundFilters(final ZuulMessage message, final Observable<ZuulMessage> chain) {
        return applyFilters(message, FilterType.OUTBOUND);
    }

    private ZuulMessage applyFilters(ZuulMessage msg, FilterType type) 
    {
    ...
    protected ZuulMessage chainFilters(ZuulMessage msg, Iterator<ZuulFilter> filterChainIterator, FilterType phase)
    {
        ...
        if (filter.getSyncType() == FilterSyncType.SYNC) {
            ...
            // Create a callback to use for continuing the filter chain after an async filter.
            FilterComplete nextFilterCallback = messageComponent -> {
                // Recurse to the next filter in chain.
                chainFilters((ZuulMessage) messageComponent, filterChainIterator, phase);  
            };

            // Apply this filter.
            msg = processSyncFilter(msg, (SyncZuulFilter) filter, nextFilterCallback, false);
        }
    }
    
    private class NextFilterCallback
    {
        private ZuulMessage currentMsg;
        private ZuulFilter currentFilter;
        private FilterExecInfo currentInfo;
        private long currentStartTime;

        public void setResult(ZuulMessage currentMsg)
        {
            this.currentMsg = currentMsg;
        }

        public void setFilter(ZuulFilter currentFilter)
        {
            this.currentFilter = currentFilter;
        }

        public void setInfo(FilterExecInfo currentInfo)
        {
        ...
        void invoke(ZuulMessage msg, Iterator<ZuulFilter> filterChainIterator, FilterType phase) 
        {
            // Record info when filter processing completes.
            recordFilterCompletion(currentMsg, currentFilter, currentInfo, currentStartTime);
            
            chainFilters(msg, filterChainIterator, phase);
        }
    }
    ...
    public ZuulMessage processSyncFilter(ZuulMessage msg, SyncZuulFilter filter, FilterComplete nextFilterCallback, boolean shouldSendErrorResponse)
    {
            ...
            if (filter.isDisabled()) {
                ...
                if (filter.shouldFilter(input)) {
                    if (filter.getSyncType() == FilterSyncType.SYNC) {
                        ...
                        if (nextFilterCallback == null) {
                            throw new IllegalArgumentException("nextFilterCallback cannot be null for AsyncFilters!");
                        }

                        nextFilterCallback.set(filter);
                        nextFilterCallback.setFilterInfo(info);
                        nextFilterCallback.setStartTime(ltime);
                        
                        ((AsyncZuulFilter) filter).applyAsync(input, nextFilterCallback);
                    }
                }
                else {
                    result = filter.getDefaultOutput(msg);
